...
module.exports.deleteBranch = async (params) => {
  const {
    accessToken,
    workspace,
    repository,
  ...
  const url = `${bitbucketBaseURL}/repositories/${workspace}/${repository}/refs/branches/${encodeURIComponent(branchName)}`;

  return bitbucketAxios(accessToken).delete(url)
}